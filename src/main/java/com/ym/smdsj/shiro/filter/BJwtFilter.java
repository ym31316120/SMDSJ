...
import com.ym.smdsj.shiro.token.JwtToken;
...
import com.ym.smdsj.util.JsonWebTokenUtil;
...
public class BJwtFilter extends BPathMatchingFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest servletRequest, ServletResponse servletResponse, Object mappedValue) throws Exception {
        ...
        LogExeManager.getInstance().executeLogTask(LogTaskFactory.bussinssLog(WebUtils.toHttp(servletRequest).getHeader("appId"),
                WebUtils.toHttp(servletRequest).getRequestURI(),WebUtils.toHttp(servletRequest).getMethod(),(byte)1,null));
            ...
            try {
                ...
                if ("expiredJwt".equals(e.getMessage())) {
                    ...
                    if (null != refreshJwt && refreshJwt.equals(jwt)) {
                        ...
                        RequestResult message = new RequestResult().success(1005,"new jwt").addData(newJwt);
                        RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
                        return false;
                    }else {
                        // jwt时间失效过期,jwt refresh time失效 返回jwt过期客户端重新登录
                        RequestResult message = new RequestResult().error(1006,"expired jwt");
                        RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
                        return false;
                    }
                ...
                // 其他的判断为JWT错误无效
                RequestResult message = new RequestResult().error(1007,"error Jwt");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
                return false;
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        if (null == subject || !subject.isAuthenticated()) {
            ...
            RequestResult message = new RequestResult().error(1006,"error jwt");
            RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
        }else {
            //  已经认证但未授权的情况
            // 告知客户端JWT没有权限访问此资源
            RequestResult message = new RequestResult().error(1008,"no permission");
            RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
        }
        // 过滤链终止
        return false;
