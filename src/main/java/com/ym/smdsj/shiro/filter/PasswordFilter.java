...
import com.ym.smdsj.util.CommonUtil;
...
public class PasswordFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (isPasswordTokenGet(request)) {
            ...
            try {
                String key = "TOKEN_KEY_"+ IpUtil.getIpFromRequest(WebUtils.toHttp(request)).toUpperCase()+userKey.toUpperCase();
                System.out.println("create red key ="+key);
                redisTemplate.opsForValue().set(key,tokenKey,100, TimeUnit.SECONDS);
                // 动态秘钥response返回给前端
                RequestResult<Map<String,String>> result = new RequestResult();
                result.success(1000,"issued tokenKey success");
                Map<String,String> keyMap = new HashMap<>();
                keyMap.put("tokenKey",tokenKey);
                keyMap.put("userKey", userKey.toUpperCase());
                result.addData(keyMap);
                RequestResponseUtil.responseWrite(JSON.toJSONString(result),response);

            }catch (Exception e) {
                LOGGER.warn("签发动态秘钥失败"+e.getMessage(),e);
                RequestResult result = new RequestResult();
                result.success(1000,"issued tokenKey fail");
                RequestResponseUtil.responseWrite(JSON.toJSONString(result),response);
            }
            return false;
        }
        ...
        if(isPasswordLoginPost(request)){
            ...
            try {
                ...
                // response 告知无效请求
                RequestResult message = new RequestResult().error(1111,"error request");
            ...
            try {
                ...
                // 返回response告诉客户端认证失败
                RequestResult message = new RequestResult().error(1002,"login fail");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
                return false;
            }catch (Exception e) {
    ...
    private AuthenticationToken createPasswordToken(ServletRequest request) throws Exception {
        ...
        String key = "TOKEN_KEY_"+host.toUpperCase()+userKey;
